...
export class Compiler {
	...
	private compileMany(node: Many): OpCode[] {
		...
		return [{op: node.nonGreedy ? "fork_next" : "fork_cont", next: codes1.length + 1}, ...codes1, {op: "jump", cont: -1 - codes1.length - 1}];
	}
	...
	private compileRepeat(node: Repeat): OpCode[] {
		...
		if (node.min === 1) {
			...
			const codes1 = this.insertCapReset(from, codes0);
			codes.push({op: "push", value: node.min}, ...codes1, {op: "dec"}, {op: "loop", cont: -1 - codes1.length - 1}, {op: "pop"});
		} else {
			this.advance = false;
		}
		...
		if (max === Infinity) {
			const codes1 = this.insertCapReset(from, this.insertEmptyCheck(codes0));
			codes.push({op: node.nonGreedy ? "fork_next" : "fork_cont", next: codes1.length + 1}, ...codes1, {op: "jump", cont: -1 - codes1.length - 1});
		} else if (max > node.min) {
			const remain = max - node.min;
			const codes1 = this.insertCapReset(from, this.insertEmptyCheck(codes0));
			if (remain === 1) {
				codes.push({op: node.nonGreedy ? "fork_next" : "fork_cont", next: codes1.length}, ...codes1);
			} else {
				codes.push(
	...
	private escapeClassToSet(node: EscapeClass): CharSet {
		switch (node.kind) {
			...
			case "unicode_property": {
				const set = loadPropertyValue("General_Category", node.property) ?? loadProperty(node.property);
				if (set === null) {
